...
bool BufferEncoder::Initialize(const mrp::EncoderConfig* config_data, BufferCallback buffer_callback){
  LOGI(" ");
...
bool BufferEncoder::EncodeBuffer(const uint8_t* yBuf, size_t ySize,
...
bool BufferEncoder::UpdateEncodingParams(uint32_t bitrate,
                                           uint32_t framerate) {
...
bool BufferEncoder::LinkElements(const EncoderConfig* configData) {
  ...
  if (PIXEL_FORMAT_I420 == configData->pixelFormat) {
    ...
    gst_bin_add_many(GST_BIN(pipeline_), source_, filter_YUY2_, parse_,
                   converter_, encoder_, filter_H264_, sink_, NULL);
#else
#if defined(USE_NV12)
  gst_bin_add_many(GST_BIN(pipeline_), source_, filter_YUY2_, parse_,
 ...
 * processing */
GstFlowReturn BufferEncoder::OnEncodedBuffer(
  ...
  if (sample) {
    ...
    if (time_past >= std::chrono::seconds(1)) {
        ...
        LOGI(": Encoder @ %d secs => %d fps", encoder->current_seconds_, encoder->buffers_per_sec_);
        encoder->start_time_ = std::chrono::system_clock::now();
